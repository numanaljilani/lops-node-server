...
const createEmployee = async (req, res) => {
  ...
  try {
    ...
    if (!contact || contact.length < 4) {
      return res.status(400).json({ message: 'Contact must be at least 4 digits' });
    }
...
export const updateEmployee = async (req, res) => {
  ...
  console.log(updates ,"update")
    ...
    if (user) {
      if (updates?.userId?.name !== undefined) user.name = updates?.userId?.name;
      if (updates?.userId?.email !== undefined) user.email = updates?.userId?.email;
...
const employeeIds = payments.map((t) => t._id);
    const tasks = await Task.find({ paymentId: { $in: employeeIds } }).select('assignTo');
