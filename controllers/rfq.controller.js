...
export const createRFQ = async (req, res) => {
  try {
    const { project_type, scope_of_work, quotation_amount, status, remarks, client, company, approvedBy } = req.body;
...
export const getRFQById = async (req, res) => {
  try {
    const rfq = await RFQ.findById(req.params.id).populate('client company approvedBy');
    if (!rfq) {
